...
const loginUser = asyncHandler(async (req, res) => {
  ...
  //Send cookies
  //options because -> It cannot be modified form the browser , It can only be modiied by the server
  const options = {
    httpOnly: true,
    secure: true,
  };

  //We are sending these tokens also , because he can also save these tokens in the localStorage (Frontend)
  return res
    .status(200)
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  // const user  = await User.find({username}) -> Expensive operation
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        // I think it should be req.body or req.params.user._id
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          ...
          {
            //Pipeline for converting array to object (For Frontend developers):
            $addFields: {
              owner: {
                $first: "$owner",
              },
